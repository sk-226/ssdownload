...
- **Wrong data source**: Using limited CSV instead of rich official site data  
- **16 duplicate entries** in cache (2136 total, 2120 unique names)
- **Missing crucial metadata**: Matrix ID, authors, dates, SVD statistics
- **Incorrect URLs**: Using unofficial Heroku app instead of official site
- **Data validation gaps**: No duplicate removal or integrity checks
- **PART file pollution**: Duplicate downloads create .part file debris
- **Wrong CSV source**: Using incorrect `sparse-files.engr.tamu.edu` instead of official `sparse.tamu.edu/files/ssstats.csv`
- **Ineffective --spd option**: Should auto-combine symmetric AND posdef conditions for SPD matrices

## Phase 1: Data Source Investigation & Design (High Priority)

### Research & Discovery
- [ ] **Switch to correct official CSV source**
  - [ ] Change from `http://sparse-files.engr.tamu.edu/files/ssstats.csv`
  - [ ] Use official `https://sparse.tamu.edu/files/ssstats.csv`
  - [ ] Compare data format and quality differences
  - [ ] Test data accuracy with known matrices (e.g., nos5)
...
- [ ] **Redesign --spd option logic**
  - [ ] Make --spd automatically imply symmetric=True AND posdef=True
  - [ ] Remove redundant spd field from data structure if not needed
  - [ ] Update filter logic to handle SPD as compound condition
  - [ ] Ensure mathematical correctness (SPD ⊆ symmetric ∩ posdef)

## Phase 2: Data Acquisition Implementation (High Priority)

### PART File Management (Critical Fix)
- [ ] **Implement pre-download file existence check**
  - [ ] Check for existing complete files before starting download
  - [ ] Clean up orphaned .part files from failed downloads
  - [ ] Add file size and checksum validation for existing files
  - [ ] Skip download if valid file already exists
- [ ] **Improve download resume logic**
  - [ ] Better .part file cleanup on successful completion
  - [ ] Add .part file age checking (clean old abandoned downloads)
  - [ ] Implement atomic file renaming (temp -> final)
  - [ ] Add user command to clean .part files: `ssdl clean-cache`

### URL and Endpoint Updates
- [ ] **Update configuration to use official sources**
  - [ ] Change CSV_INDEX_URL to `https://sparse.tamu.edu/files/ssstats.csv`
  - [ ] Change base URLs from Heroku to official sparse.tamu.edu
  - [ ] Update file download endpoints
  - [ ] Update checksum URL generation
